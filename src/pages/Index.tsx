...
const Index = () => {
    ...
    try {
      ...
      if (exportFormat === "video" || exportFormat === "gif") {
        ...
        if (isGif) {
          ...
          await loadGifJs();
          const gif = new (window as any).GIF({ workers: 2, quality: 10, width: frames[0].width, height: frames[0].height, transparent: transparent ? "rgba(0,0,0,0)" : null });
          frames.forEach(frame => gif.addFrame(frame, { delay: 1000 / fps, copy: true }));
          gif.on('finished', (blob: Blob) => {
            const link = document.createElement('a'); link.download = `animation.gif`; link.href = URL.createObjectURL(blob); link.click();
            setExporting(false);
          });
          gif.render();
        } else {
          ...
          recorder.onstop = () => {
            const link = document.createElement('a'); link.download = `video.webm`; link.href = URL.createObjectURL(new Blob(chunks)); link.click();
            setExporting(false);
          };
          recorder.start();
          let f = 0;
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#f1f5f9] dark:bg-[#020617] text-foreground transition-colors duration-700 flex flex-col overflow-hidden font-sans">
      ...
      {exporting && (
        <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-xl flex flex-col items-center justify-center gap-4">
           <div className="w-[300px] space-y-4">
              <div className="flex justify-between items-end"><Label className="text-xl font-black uppercase">{exportStatus}</Label><span className="text-xs font-mono">{exportProgress}%</span></div>
      ...
      <div className="flex-1 flex min-h-0 relative">
                ...
                <AccordionContent className="pt-4 space-y-6">
                  <div className="bg-muted/30 p-3 rounded-xl space-y-3">
                    ...
                    <Select value={exportFormat} onValueChange={(v: any) => setExportFormat(v)}>
                       ...
                       <SelectContent>
                         {mode === "mockup" ? (<><SelectItem value="png">PNG (Lossless)</SelectItem><SelectItem value="jpeg">JPG (Compressed)</SelectItem></>) : (<><SelectItem value="gif">GIF</SelectItem><SelectItem value="video">MP4</SelectItem></>)}
                       </SelectContent>
                    </Select>
                  </div>
                        ...
                        <div className="space-y-6">
                           ...
                           <div className="space-y-6 px-1">
                              <div className="space-y-3">
                                ...
                                <div className="space-y-1"><div className="flex justify-between items-center"><Label className="text-[8px] opacity-30 uppercase">Scale</Label><span className="text-[8px] opacity-30 font-mono">{animStartScale}%</span></div><Slider value={[animStartScale]} onValueChange={(v) => setAnimStartScale(v[0])} min={10} max={150} /></div>
                              </div>
                              <div className="space-y-3">
                                ...
                                <div className="space-y-1"><div className="flex justify-between items-center"><Label className="text-[8px] opacity-30 uppercase">Scale</Label><span className="text-[8px] opacity-30 font-mono">{animEndScale}%</span></div><Slider value={[animEndScale]} onValueChange={(v) => setAnimEndScale(v[0])} min={10} max={150} /></div>
                              </div>
        ...
        <main className="flex-1 flex flex-col relative overflow-hidden bg-muted/5">
          <div ref={stageRef} className="flex-1 flex items-center justify-center relative overflow-hidden p-8">
